...
describe('c3 chart class', function () {
    ...
    describe('internal.generateTargetClass', function () {
        it('should not replace any characters', function () {
            var input = 'data1', expected = '-' + input, suffix = chart.internal.generateTargetClass(input);
            expect(suffix).toBe(expected);
        });
        it('should replace space to "-"', function () {
            var input = 'data1 suffix', expected = '-data1-suffix', suffix = chart.internal.generateTargetClass(input);
            expect(suffix).toBe(expected);
        });
        it('should replace space to "-" with multibyte characters', function () {
            var input = 'data1 suffix 日本語', expected = '-data1-suffix-日本語', suffix = chart.internal.generateTargetClass(input);
            expect(suffix).toBe(expected);
        });
        it('should not replace special characters', function () {
            var input = 'data1 !@#$%^&*()_=+,.<>"\':;[]/|?~`{}\\', expected = '-data1-!@#$%^&*()_=+,.<>"\':;[]/|?~`{}\\', suffix = chart.internal.generateTargetClass(input);
            expect(suffix).toBe(expected);
        });
    });
    describe('internal.getTargetSelectorSuffix', function () {
        it('should escape special characters', function () {
            var input = 'data1 !@#$%^&*()_=+,.<>"\':;[]/|?~`{}\\', expected = '-data1-\\!\\@\\#\\$\\%\\^\\&\\*\\(\\)_\\=\\+\\,\\.\\<\\>\\"\\\'\\:\\;\\[\\]\\/\\|\\?\\~\\`\\{\\}\\\\', suffix = chart.internal.getTargetSelectorSuffix(input);
            expect(suffix).toBe(expected);
        });
    });
    describe('select target in chart', function () {
        ...
        it('should be able to select class with unicode characters', function () {
            var selector = ".c3-target" + chart.internal.getTargetSelectorSuffix(args.data.columns[3][0]);
            expect(chart.internal.main.select(selector).size()).toBe(1);
        });
    });
});
