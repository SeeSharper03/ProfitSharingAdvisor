...
describe('c3 chart tooltip', function () {
    ...
    describe('tooltip position', function () {
        ...
        describe('without left margin', function () {
            it('should show tooltip on proper position', function () {
                var eventRect = d3.select('.c3-event-rect').node(), x = chart.internal.x(1), y = chart.internal.y(200);
                setMouseEvent(chart, 'mousemove', x, y, eventRect);
                var tooltipContainer = d3.select('.c3-tooltip-container'), top = Math.floor(+tooltipContainer.style('top').replace(/px/, '')), left = Math.floor(+tooltipContainer.style('left').replace(/px/, ''));
        ...
        describe('with left margin', function () {
            ...
            it('should show tooltip on proper position', function () {
                var eventRect = d3.select('.c3-event-rect').node(), x = chart.internal.x(1) + 300, // add margin-left
    ...
    describe('tooltip positionFunction', function () {
        ...
        it('should be set to the coordinate where the function returned', function () {
            var eventRect = d3.select('.c3-event-rect').node(), x = chart.internal.x(2), y = chart.internal.y(100);
            setMouseEvent(chart, 'mousemove', x, y, eventRect);
            var tooltipContainer = d3.select('.c3-tooltip-container'), top = Math.floor(+tooltipContainer.style('top').replace(/px/, '')), left = Math.floor(+tooltipContainer.style('left').replace(/px/, ''));
            expect(top).toBeGreaterThan(0);
            expect(left).toBeGreaterThan(0);
    ...
    describe('tooltip with data_order as desc with grouped data', function () {
        ...
        it('should display each data in descending order', function () {
            var eventRect = d3.select('.c3-event-rect').node(), x = chart.internal.x(2), y = chart.internal.y(220);
            setMouseEvent(chart, 'mousemove', x, y, eventRect);
            var classes = d3
                .selectAll('.c3-tooltip tr')
                .nodes()
